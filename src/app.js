...
async function callAiInsightsProxy(summary, businessContext) {
  const response = await fetch('/api/ai-insights', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ summary, businessContext }),
  });
  let data = null;
...
async function generateAiInsights() {
  ...
  try {
    ...
    const text = await callAiInsightsProxy(summary, state.businessContext);
...
function updateUploadRecap() {
  ...
  if (el) el.innerHTML = `Analyzing for <b>${escapeHtml(domainLabel())}</b> — "${escapeHtml(state.businessContext.description)}"`;
}
...
async function saveToStorage(rawRows, columns, meta) {
  ...
  try {
    const payload = JSON.stringify({ rawRows, columns, meta, businessContext: state.businessContext });
...
function renderActiveTab() {
  ...
  TAB_RENDERERS[tab](getFilteredRows());
...
async function init() {
  ...
  if (stored && stored.rawRows && stored.rawRows.length && stored.columns) {
